...
// #### @emailObj {object} If set, will return a formatted string of data
// #### @callback {function} Continuation to respond to when complete.
// Returns the list of all process data managed by forever.
//
forever.sendEmail = function (callback) {
  if(forever.email){
    //variables needed for email support
    var json = JSON.parse(fs.readFileSync(forever.email, 'utf8'));
